...
<!--- /////////////  FOR LISTING MANAGMENT ///////////////// --->
	   	...
	   	<cfquery name="qListings" datasource="#application.dsource#">
			...
			<cfif isDefined('arguments.manufacturer') AND arguments.manufacturer neq ''>
				<cfset decodedmanufacturer = URLDecode(arguments.manufacturer)>
	      		...
	      		AND manufacturer like '#decodedmanufacturer#%'
	      	</cfif>
			<cfif isDefined('arguments.path') AND arguments.path neq ''>
	      		<cfset decodedPath = URLDecode(arguments.path)>
	...
	<cffunction name="editListingsFromForm" access="remote" output="false" returntype="struct">
        ...
        <cfargument name="ARTTYPE" type="string" default="">
	    ...
	    <!--- <cfset var success = true /> --->
			...
			<cfif NOT len(arguments.LOCATION_PRICE)><cfset arguments.LOCATION_PRICE = 0 /></cfif>
			 ...
			 <cfset var result = { success = true, message = "", imageValidationFailed = false }>
			 <cfset var hasInvalidImageExtension = false>
			...
			<cftry>
				...
				<cfif len(arguments.thisImage)>
                	...
                	<cfif fileupload.fileWasSaved AND listFindNoCase("jpg,jpeg", fileExt)>
					 ...
					 <cfelse>
						...
						<cfset hasInvalidImageExtension = true>
				...
				<cfloop collection="#form#" item="idx">
					<cfif left(idx,9) EQ "addImage_">
						...
						<cfif len(trim(currImage)) GT 0>
							...
							<cfset fileExt = lcase(fileuploadTemp.clientFileExt)>

							<cfif NOT listFindNoCase("jpg,jpeg", fileExt)>
								<cffile action="delete" file="#fileuploadTemp.serverDirectory#/#fileuploadTemp.serverFile#">
								<cfset hasInvalidImageExtension = true>
							<cfelse>
								<!--- Store valid JPEG uploads using the app's existing JPG naming convention. --->
								<cfset thisImageId = "#thisId#_#addImageIdx#.jpg" />
				...
				<cfif arrayLen(uploadedImages) GT 0>
				...
				<cfif hasInvalidImageExtension>
					<cfset result.imageValidationFailed = true>
					<cfset result.message = result.message & " One or more image uploads were skipped because only JPG or JPEG files are allowed.">
				</cfif>
    ...
    <cffunction name="getAllArtType" access="remote" returntype="query" >
        <cfset var success = true />
		<cfset var qArtType = '' />

        <cftry>

            <cfquery name="qArtType" datasource="#application.dsource#">
                   SELECT distinct artType  FROM products
                ORDER BY artType
            </cfquery>
    ...
    <cffunction name="getAllArtSize" access="remote" returntype="query" >
        <cfset var success = true />
		<cfset var qArtSize = '' />

        <cftry>

            <cfquery name="qArtSize" datasource="#application.dsource#">
                   SELECT distinct artSize  FROM products
                ORDER BY artSize
            </cfquery>
    ...
    <cffunction name="getAllArtSubject" access="remote" returntype="query" >
        <cfset var success = true />
		<cfset var qArtSubject = '' />

        <cftry>

            <cfquery name="qArtSubject" datasource="#application.dsource#">
                   SELECT distinct artSubject  FROM products
                ORDER BY artSubject
            </cfquery>
...
<cffunction name="deleteArtListing" access="remote" output="false" returntype="any">
 ...
 <!--- /////////////  FOR SELLER MANAGMENT ///////////////// --->
	...
	<cffunction name="getCsvListingsByArtist" access="remote" returntype="query">
		...
		<cfquery name="qListings" datasource="#application.dsource#">
           	SELECT modelno,manufacturer,name,gallery_price,uid,year FROM products P
			WHERE P.active = 1
			AND
	...
	<cffunction name="getListingsMassUpdate" access="remote">
		...
		<cfargument name="artType" required="no" type="string" default="">
	...
	<cffunction name="massUpdateProc" access="remote">
        <!--- <cfdump var="#form.updatedRowRecord#" abort="true"> --->
        ...
        <cfif isDefined('form.updatedRowRecord') and form.updatedRowRecord neq '' >			
            ...
            <cfquery name="update" datasource="#application.dsource#">
                ...
                <cfif isDefined('form.artType_'&this_keyval)>
                    ,artType = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.artType_'&this_keyval)#">
                </cfif>

                <cfif isDefined('form.artSize_'&this_keyval)>
                    ,artSize = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.artSize_'&this_keyval)#">
                </cfif>

                <cfif isDefined('form.artSubject_'&this_keyval)>
                    ,artSubject = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.artSubject_'&this_keyval)#">
                </cfif>
                <cfif isDefined('form.artTypee_'&this_keyval)>
                    ,artTypee = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.artTypee_'&this_keyval)#">
                </cfif>
                <cfif isDefined('form.location_notes_'&this_keyval)>
                    ,location_notes = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.location_notes_'&this_keyval)#">
                </cfif>
                <cfif isDefined('form.caption_'&this_keyval)>
                    ,caption = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#evaluate('form.caption_'&this_keyval)#">
                </cfif>
                <cfif isDefined('form.quantity_'&this_keyval)>
                    ,quantity = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#evaluate('form.quantity_'&this_keyval)#">
                </cfif>
                ...
                <cfif isDefined('form.Slideshow_'&this_keyval)>
                    ,Slideshow = <cfqueryparam cfsqltype="CF_SQL_TINYINT" value="1">
                <cfelse>
                    ,Slideshow = 0
                </cfif>
                <cfif isDefined('form.Frontshow_'&this_keyval)>
                    ,frontshow = <cfqueryparam cfsqltype="CF_SQL_TINYINT" value="1">
                <cfelse>
                    ,frontshow = 0
                </cfif>
                <cfif isDefined('form.BottomHome_'&this_keyval)>
                    ,family = <cfqueryparam cfsqltype="CF_SQL_TINYINT" value="1">
                <cfelse>
                    ,family = 0
                </cfif>
